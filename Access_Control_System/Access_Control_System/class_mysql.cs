...
namespace Access_Control_System
{
    class class_mysql
    {
        ...
        //資料庫初始化設定
        public void Initialize(string host, string database, string user, string pwd)
        {
            dbhost = host;//host位址
            dbuser = user;//帳號
            dbpass = pwd;//密碼
        ...
        //查詢客戶資料
        public ListView select_customer(ListView listView_customer)
        {
        ...
        //查詢客戶活動
        public ListView select_customer_activity(ListView listView_customer_activity)
        {
        ...
        //新增客戶活動
        public void insert_customer_activity(string key, string card_id, int state)
        {
        ...
        //查詢刷卡
        public DataTable select_customer_swipe(string card_id)
        {
        ...
        //新增客戶
        public void insert_customer_added(string name, string card_id, int permission, int state, string image)
        {
            string sql = "INSERT INTO oc_customer (`name`, `card_id`, `permission`, `state`, `image`, `join_time`) VALUES ('" + name + "', '" + card_id + "', '" + permission + "', '" + state + "', '"+ image +".jpg', NOW())";
            MySqlCommand cmd = new MySqlCommand(sql, conn);
            cmd.ExecuteNonQuery();
        }

        //更改客戶資料
        public void update_customer(string name, string card_id, int permission, int state, string image)
        {
            string sql = "UPDATE oc_customer SET name = '" + name + "', permission = '" + permission + "', state = '" + state + "', image = '" + image + ".jpg' WHERE card_id = '" + card_id + "'";
            MySqlCommand cmd = new MySqlCommand(sql, conn);
            cmd.ExecuteNonQuery();
        }

        //刪除客戶
        public void delete_customer(string card_id)
        {
